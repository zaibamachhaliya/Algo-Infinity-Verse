...
/* global practiceProblems, userProgress, openQuizEditor, openPracticeFromPath */
...
function buildRoadmap(goal) {
  ...
  container.innerHTML = '';
  lastMilestone =
    Math.floor(checked.length > 0 ? ((checked.length / topics.length) * 100) / 25 : 0) * 25;
  ...
  document.querySelectorAll('.step-checkbox').forEach((cb) => {
    cb.addEventListener('change', () => {
      ...
      if (cb.checked) {
        ...
        if (dot)
          dot.innerHTML =
            '<i class="fas fa-check" style="color: #fff;" aria-hidden="true" focusable="false"></i>';
        const labelEl = step.querySelector('.step-icon-label');
        if (labelEl)
          labelEl.innerHTML =
            '<i class="fas fa-check-circle" style="color: #22c55e;" aria-hidden="true" focusable="false"></i>';
        ...
        if (dot)
          dot.innerHTML =
            '<i class="fas fa-circle" style="color: var(--primary); font-size: 0.6rem;" aria-hidden="true" focusable="false"></i>';
        const labelEl = step.querySelector('.step-icon-label');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Delegated open path from practice link
  document.addEventListener('click', (e) => {
    const el = e.target.closest('a[data-action="open-path"]');
    if (el) {
      openPracticeFromPath(parseInt(el.dataset.pathId));
    }
  });
});
...
function generateDynamicPath() {
  ...
  const categoryMetadata = {
    ...
    strings: {
      ...
      icon: '<i class="fas fa-quote-right" style="color: #38bdf8;"></i>',
      difficulty: 'easy',
      baseHours: 8,
      link: '/pages/learning/trie-string-learning/trie-string-learning.html',
      desc: 'Anagrams, palindromes, substring searches, tries',
    },
    linkedlist: {
      name: 'Linked Lists',
      icon: '<i class="fas fa-link" style="color: #22c55e;"></i>',
    ...
    graphs: {
      ...
      baseHours: 14,
      link: '/pages/learning/graph-learning/graph-learning.html',
      desc: 'DFS/BFS, Dijkstra, Bellman-Ford, topological sort',
    },
    dp: {
  ...
  const customFocus = (document.getElementById('customRefinementText')?.value || '')
    .trim()
    .toLowerCase();
  ...
  PATHS.personalized = recommendedCategories.map((cat) => {
    ...
    const recsHtml =
      problemRecs.length > 0
        ? `<div style="margin-top: 0.5rem; font-size: 0.82rem; color: var(--accent);">
          <i class="fas fa-tasks"></i> Recommended practice: ${problemRecs
            .map(
              (p) => `
            <a href="javascript:void(0)" data-path-id="${p.id}" data-action="open-path" style="color:var(--accent); font-weight:600; text-decoration:underline; margin-right: 0.4rem;">${p.title}</a>
          `
            )
            .join(' ')}
         </div>`
  ...
  container.innerHTML = '';
  lastMilestone =
    Math.floor(checked.length > 0 ? ((checked.length / PATHS.personalized.length) * 100) / 25 : 0) *
    25;
