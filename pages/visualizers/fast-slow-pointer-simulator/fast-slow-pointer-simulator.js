...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function prepareSimulation() {
    ...
    simSteps.push({
      ...
      slowDist,
      fastDist,
      message: 'Initialize pointers at head.',
      phase: 'init',
    });
      ...
      while (fast !== null && nodes[fast].next !== null) {
        ...
        if (slow === fast) {
          ...
          simSteps.push({
            ...
            slowDist,
            fastDist,
            message: 'Moving slow by 1, fast by 2.',
            phase: 'search',
          });
        }
      }
    ...
    if (met && algo === 'find-entry') {
      ...
      simSteps.push({
        ...
        slowDist,
        fastDist,
        message: 'Reset slow to head to find entry point.',
        phase: 'entry-search',
      });
      while (slow !== fast) {
        slow = nodes[slow].next;
        ...
        if (slow === fast) {
          simSteps.push({
            ...
            slowDist,
            fastDist,
            message: 'Pointers met again! This is the cycle entry.',
            phase: 'done',
          });
        } else {
          simSteps.push({
            ...
            slowDist,
            fastDist,
            message: 'Moving both pointers by 1 step.',
            phase: 'entry-search',
          });
        }
      }
    ...
    if (met && algo === 'cycle-length') {
      ...
      simSteps.push({
        ...
        slowDist,
        fastDist,
        message: 'Keep slow stationary, move fast by 1 to count length.',
        phase: 'count',
      });
      ...
      simSteps.push({
        ...
        slowDist,
        fastDist,
        message: `Cycle length is ${length}.`,
        phase: 'done',
      });
    }
      ...
      while (fast !== null && nodes[fast].next !== null) {
        ...
        if (fast === null || nodes[fast].next === null) {
          simSteps.push({
            ...
            slowDist,
            fastDist,
            message: 'Fast reached end. Slow is at the middle.',
            phase: 'done',
          });
          break;
        } else {
  ...
  function updateUI() {
    ...
    if (step.fastIdx !== null && nodes[step.fastIdx]) {
      ...
      if (step.slowIdx === step.fastIdx) {
        document.getElementById(`node-${step.fastIdx}`).className =
          'll-node-wrapper highlight-both';
      } else {
